...
#        pnts = np.random.rand(n,2)
        ...
        for i in range (0, nbatch):
            ...
            for j in range (0, pnts.shape[0]):
                pnt = pnts[j].reshape(2,1)
                pnt_K = K.variable(value=pnt)
                pnt_pred = K.dot(y_pred_mat, pnt_K)
                pnt_true = K.dot(y_true_mat, pnt_K)
                err += K.mean( K.square(pnt_pred - pnt_true ) )
 
        err /= ( nbatch * pnts.shape[0] )
        w = 1.0/(np.linalg.norm(pnts))
        print('w for regularization term is ', w)
        return K.variable(value=err) + w * K.mean(K.square(y_true-y_pred) )
    return loss
...
#    model.compile(loss='mse', optimizer='adam', metrics=['mse'])
    model.compile(loss=customLoss( pnts, nbatch=nbatch), optimizer='adam', metrics=['mse'])
...
#    storeModel = w_path + 'train_all/model_ws5_temporal.h5'
    ...
    if (reTrain):
        ...
        n = 60 #number of points in one cross-section
        ...
        model = buildModel(nb_features, nb_outputs, neurons, w_path, nbatch, pnts)
