...
def buildModel(window_size, filter_length, nb_features, nb_output, nb_filter):
    model = Sequential()
    model.add(Convolution1D(nb_filter=nb_filter, filter_length=filter_length, activation='relu', input_shape=(window_size, nb_features), padding = 'same'))
#    model.add(MaxPooling1D()) #default pool_size is 2
    model.add(Convolution1D(nb_filter=nb_filter, filter_length=filter_length, activation='relu', padding = 'same'))
#    model.add(MaxPooling1D())
    model.add(Convolution1D(nb_filter=nb_output, filter_length=filter_length, activation='relu', padding = 'same'))
#    model.add(MaxPooling1D())
#    Flatten()
...
def loadData(fn_trainX, fn_trainY, nb_features, nb_output, window_size):
    ...
    assert (Y_train_all.shape[1] == window_size*nb_output)
    ...
    Y_train = all_train[0:nb_halfdata, -window_size*nb_output:]   
    X_valid = all_train[nb_halfdata:, 0: window_size*nb_features]
    Y_valid = all_train[nb_halfdata:, -window_size*nb_output:] 
    
    # x-shape: nb_halfdata, window_size, nb_features
    X_train_ = X_train.reshape(nb_halfdata, window_size, nb_features)
    Y_train_ = Y_train.reshape(nb_halfdata, window_size, nb_output)
    X_valid_ = X_valid.reshape(nb_instance-nb_halfdata, window_size, nb_features)
    Y_valid_ = Y_valid.reshape(nb_instance-nb_halfdata, window_size, nb_output)
    ...
    return X_train_, Y_train_, X_valid_, Y_valid_, nb_instance
...
def evaluate(nb_features, window_size, nb_output):
    ...
    model.fit(X_train, Y_train, nb_epoch=1, batch_size=2, validation_data=(X_valid, Y_valid))
...
window_size = 9   
nb_output = 4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     
nb_output, model = evaluate(nb_features, window_size, nb_output)
