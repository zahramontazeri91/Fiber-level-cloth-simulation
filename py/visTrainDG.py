...
for i in range (firstFrame/skipFactor,lastFrame/skipFactor + 1):  
    f = i * skipFactor
    fn_train = path + 'pattern/yarn4/spacing1.0x/00011/physical_' + str(f) + '_0.txt'
    if (isFirst):
...
for i in range (firstFrame/skipFactor,lastFrame/skipFactor + 1):  
    f = i * skipFactor
    fn_test = path + 'woven/yarn4/spacing1.0x/00011/physical_' + str(f) + '_0.txt'
    if (isFirst):
...
ax = Axes3D(fig, elev=90, azim=0)

X_reduced_train = PCA(n_components=3).fit_transform(X_train)
...
X_reduced_test = PCA(n_components=3).fit_transform(X_test)
test = ax.scatter(X_reduced_test[:, 0], X_reduced_test[:, 1], X_reduced_test[:, 2], c='r',
           cmap=plt.cm.Set1, edgecolor='k', s=70, label='test data - woven 0', alpha=0.2)
...
plt.savefig('../../data/test_train.jpg') 
plt.show()