...
##################################
def buildTrainY(nb_seg, trimPercent, first_frame, last_frame, filename):
    for i in range (first_frame,last_frame+1):
        fname_write = path + filename + str(i-first_frame) + '.txt'
...
def buildTrainX_conv(nb_seg, trimPercent, first_frame, last_frame, sigma, filename):
    for i in range (first_frame,last_frame+1):
        fname_write = path + filename + str(i-first_frame) + '.txt'
        with open(fname_write, 'w') as fout:
            if i < 10 :
...
sigma = 1
filename = 'trainY_'
buildTrainY(nb_seg, trimPercent, first_frame, last_frame, filename)
filename = 'trainX_'
buildTrainX_conv(nb_seg, trimPercent, first_frame, last_frame, sigma, filename)
