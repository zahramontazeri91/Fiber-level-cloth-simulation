...
def main_batch(args):
    ...
    for (i, pattern) in enumerate(patterns):
        for (j, spacing) in enumerate(spacings):
            ...
            hairs =  parse_fiber_bundle(args['input'])
            print(len(hairs))
            ...
            hair_counter=[]
            f = open(outname, 'w+')
            record_name = 'results/yarn100/spacing{}/{}/fiber/'.format(spacing_names[j], names[i] )
            # print(record_name)
...
if __name__ == '__main__':
	...
	write_head(f, record_filename='results/yarn100/spacing3.0x/10/fiber/')
	write_cylinder_with_pattern(f, spacing=0.8, pattern=[1,0])
	write_script(f)
	hair_counter = write_fibers(f=f, hairs=hair1, hair_counter=hair_counter, fid=0, is_tip_fixed=True)
	write_hair_collections(f, hair_counter)
	write_end(f)
