...
def write_head(f, record_filename):
    ...
    f.write('</StrandParameters>\n')
    f.write('<savetag geometry="y" internal_force="y" twist="y" fe="y" attachforce="y" distance_field="y"/>\n')
    
def write_script(f):
    f.write('<script type="translate" x="0.0" y="0.0" z="-3.0" w="0.0" start="0.0" end="2.0" group="1"/> \n')
...
def write_single_hair(f, hair, hair_counter, fid, is_tip_fixed):
    for i in range(hair.shape[0]):
        if (is_tip_fixed and (i==0)):
            f.write('<particle x="'+str(hair[i, 0])+' '+str(hair[i, 1])+' '+str(hair[i, 2])+'" v="0.0 0.0 0.0" fixed="1" group="1" fid="'+str(fid)+'"/>\n')
        elif (is_tip_fixed and (i==hair.shape[0]-1)):
            f.write('<particle x="'+str(hair[i, 0])+' '+str(hair[i, 1])+' '+str(hair[i, 2])+'" v="0.0 0.0 0.0" fixed="1" group="2" fid="'+str(fid)+'"/>\n')
        else:
...
if __name__ == '__main__':
    ...
    write_head(f, record_filename='results/yarn11/stretch/')
    # write_head(f, record_filename='results/speed/1.5_10/1.0_both/')
    # write_cylinder_with_pattern(f, spacing=0.7, pattern=[0,0,0,1,1])
    write_script(f)
    hair_counter = write_fibers(f=f, hairs=hair1, hair_counter=hair_counter, fid=0, is_tip_fixed=True)
    write_hair_collections(f, hair_counter)
    write_end(f)
