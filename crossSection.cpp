...
void CrossSection::write_PlanesIntersections2D(const char* filename, std::vector<yarnIntersect> &itsLists, std::vector<yarnIntersect2D> &allPlaneIntersect) {
...
double CrossSection::getOrientation(const yarnIntersect2D &pts, vec2f &center, vec2f &p1, vec2f &p2)
{
	...
	//	data_pts.at<float>(i, 0) = i;
	//	data_pts.at<float>(i, 1) = 1.0;
	//}
	...
	center = vec2f(pca_analysis.mean.at<float>(0, 0), pca_analysis.mean.at<float>(0, 1));
	...
	p1 = center + vec2f(eigen_vecs[0].x * eigen_val[0], eigen_vecs[0].y * eigen_val[0]);
	p2 = center - vec2f(eigen_vecs[1].x * eigen_val[1], eigen_vecs[1].y * eigen_val[1]);

	double angle = atan2(eigen_vecs[0].y, eigen_vecs[0].x); // orientation in radians

	std::cout << " center " << center.x << " " << center.y << std::endl;
	std::cout << " eigen_vec_0 " << eigen_vecs[0].x << " " << eigen_vecs[0].y << std::endl;
	std::cout << " eigen_vec_1 " << eigen_vecs[1].x << " " << eigen_vecs[1].y << std::endl;
	std::cout << " eigen_val " << eigen_val[0] << " " << eigen_val[1] << std::endl << std::endl;
	return angle;
}
...
void CrossSection::extractCompressParam(const std::vector<yarnIntersect2D> &allPlaneIntersect, std::vector<Ellipse> &ellipses, const char* compressFile) {
	...
	for (int i = 0; i < allPlaneIntersect.size(); ++i)
	{
		...
		Ellipse e;
		getOrientation(allPlaneIntersect[i], e.center, e.longP, e.shortP);
